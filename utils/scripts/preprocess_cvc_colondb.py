...
DATASET_ROOT = r"D:\dataset\CVC-ColonDB"
OUTPUT_DIR = r"E:\code\Med-CLIP-SAM\data\processed\CVC_ColonDB_256_Expert_Std"
...
# 统一的专家描述，帮助文本编码器提取一致的特征
CVC_COLONDB_EXPERT_DESC = "colon polyp, a protruding growth on the inner lining of the colon, appearing reddish and textured"

# ================= 3. Core Functions =================

def remove_specular_reflections(img_bgr):
    """
    🌟 针对 ColonDB 强反光优化的去高光修补
...
def preprocess_polyp_enhancement(img_bgr):
    ...
    # 3. CLAHE on L-channel (仅增强亮度通道，保留真实病灶色彩)
    l, a, b = cv2.split(lab)
...
def process_single_pair(args):
    ...
    try:
        ...
        return {
            ...
            "description": CVC_COLONDB_EXPERT_DESC,
            "organ": ORGAN_NAME,
...
def main():
    ...
    print(f"🚀 Starting CVC-ColonDB Preprocessing (A-Class Unseen Dataset)")
    print(f"✨ Strategy: Fast Specular Removal + LAB CLAHE")
    print(f"⚡ Workers: {NUM_WORKERS}")

    # Path setup based on user snapshot
    ...
    # 过滤可能存在的 Mac 系统隐藏文件
    image_files = [f for f in glob.glob(os.path.join(images_dir, "*.png")) if not os.path.basename(f).startswith("._")]
    print(f"✅ Found {len(image_files)} images, ready to process...")
